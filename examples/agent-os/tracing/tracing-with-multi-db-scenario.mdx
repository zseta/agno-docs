...
```python tracing_with_multi_db_scenario.py
"""
Traces with AgentOS
Requirements:
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U "agno[os]" ddgs fastmcp openai opentelemetry-api opentelemetry-exporter-otlp starlette
    ```
  </Step>
  ...
  <Step title="Run the example">
    Save the code above as `tracing_with_multi_db_scenario.py`, then run:
    ```bash
    python tracing_with_multi_db_scenario.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/tracing/06_tracing_with_multi_db_scenario.py](https://github.com/agno-agi/agno/blob/main/cookbook/05_agent_os/tracing/06_tracing_with_multi_db_scenario.py)
