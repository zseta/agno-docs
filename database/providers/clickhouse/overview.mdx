...
- **Low-cardinality filters.** Filtering by `status`, `agent_id`, `span_kind` over billions of rows is what `LowCardinality(String)` and the merge tree index are built for.
