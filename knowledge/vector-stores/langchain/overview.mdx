...
```shell
uv pip install langchain langchain-community langchain-openai langchain-chroma
```
...
<Note>
`LangChainVectorDb` is search-only. `insert()` and `upsert()` raise `NotImplementedError`. Load documents through LangChain, then wire the retriever to Agno. Pass metadata filters as a dictionary; filter expressions are not supported.
</Note>
