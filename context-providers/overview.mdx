...
<Tabs>
  <Tab title="default">
    ...
    **How it works:** Two sub-agents handle the underlying toolkit. Reads go through the read sub-agent (history, threads, lookups; search when a user token or Slack-interface token is configured). Writes go through the write sub-agent (send_message + lookups for channel resolution).
  </Tab>
  <Tab title="agent">
