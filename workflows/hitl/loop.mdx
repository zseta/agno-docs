...
`timeout` and `on_timeout` only apply to Step-level pauses. Loop pauses (the start confirmation and iteration review) wait until they are resolved. See [Timeout](/workflows/hitl/timeout).
