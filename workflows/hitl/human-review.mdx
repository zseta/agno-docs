...
Condition and Steps only support `requires_confirmation`. Parallel supports no HITL fields. Passing unsupported fields raises a clear error:
...
Set a timeout for how long to wait for a user response. If the timeout expires, `on_timeout` determines what happens. Timeout is applied by Step pauses; other components ignore it.
