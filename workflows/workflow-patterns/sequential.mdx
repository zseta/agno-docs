...
If you use a custom function as an executor for a step, make sure its input and output types are compatible with the `StepInput` and `StepOutput` interfaces so the function integrates into the workflow system.
