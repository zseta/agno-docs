...
Postgres reads the password only when the `pgdata` volume is first initialized, so on a host that already ran the dev Compose the database keeps the old password and the API blocks waiting for it. Change it in place and set `.env` to match:
