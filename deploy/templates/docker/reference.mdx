...
Apply with `docker compose up -d` (in production, `docker compose -f compose.yaml -f compose.prod.yaml up -d`). The interface activates automatically and routes messages to Agent Builder; change the `agent=` argument in `app/main.py` to point at another agent. See [Slack setup](/deploy/interfaces/slack/overview).
  </Accordion>
...
| `AGENTOS_URL` | No | `http://127.0.0.1:8000` | Scheduler base URL. Set it in `.env` to your public URL; `compose.prod.yaml` passes it through. Left at the default in production, the daily deployment check flags the platform as misconfigured. |
