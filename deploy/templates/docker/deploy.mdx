...
| claude.ai and ChatGPT | Hosted sessions can't reach localhost. Run in production first, then register `https://<your-public-url>/mcp` as a remote MCP server; sign-in uses OAuth. |
...
`AGENTOS_URL` is the address the platform advertises as its own. Left unset, the daily deployment check flags the platform as misconfigured, and chat-app connectors have nothing to point at. `DB_PASS` replaces the dev default (`ai`).
...
For claude.ai and ChatGPT on the web, add `https://<your-public-url>/mcp` as a custom connector in the chat app's connector settings and sign in with OAuth.
  </Step>
