...
sidebarTitle: "Deploy"
description: "Set up the self-hosted Starter with a coding-agent prompt, run it on Docker, and go to production with one Compose override on a host you control."
---
...
| claude.ai and ChatGPT (web) | Hosted sessions can't reach localhost. Run in production first, then register `https://<your-public-url>/mcp` as a remote MCP server; sign-in uses OAuth. |
...
If `AGENTOS_URL` stays unset, the daily deployment check flags the platform as misconfigured, and chat-app connectors have nothing to point at. `DB_PASS` replaces the dev default (`ai`).
...
Token-Based Authorization is on by default; without a `JWT_VERIFICATION_KEY` or `JWT_JWKS_FILE`, the platform refuses to serve production traffic.
...
For claude.ai and ChatGPT on the web, register `https://<your-public-url>/mcp` as a remote MCP server and sign in with OAuth.
  </Step>
</Steps>
...
The same middleware protects `/mcp`: clients authenticate with a bearer token, either a JWT minted at os.agno.com or the service-account token that `uvx agno connect --url` mints. To opt out inside a private network behind another auth layer, set `authorization=False` in `app/main.py` and restart.
