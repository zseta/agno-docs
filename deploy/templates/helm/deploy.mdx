...
| claude.ai and ChatGPT | Hosted sessions can't reach localhost. Deploy to production first, then register `https://<your-agentos-domain>/mcp` as a remote MCP server; sign-in uses OAuth. |
...
For claude.ai and ChatGPT on the web, add `https://<your-agentos-domain>/mcp` as a custom connector in the chat app's connector settings and sign in with OAuth.
  </Step>
...
Changed secrets roll the pod automatically.
