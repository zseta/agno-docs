...
description: "The bundled skills that let Claude Code, Codex, and Cursor drive the create, improve, evaluate, maintain lifecycle."
---
...
Everything on this page runs inside a cloned template. If you don't have one running yet, [pick a Starter](/deploy/templates) first.
...
It auto-detects Claude Code, Claude Desktop, Codex, and Cursor, registers `http://localhost:8000/mcp`, and verifies the connection with a real handshake. Your coding agent can then call `run_agent`, `run_team`, and `run_workflow` against the live platform: build an agent with `/create-new-agent`, then test it over MCP in the same session.
...
| Context provider | `agents/platform_manager.py` | Collapse many tools into one `query_<thing>` interface that hands off to a sub-agent. |
...
The scheduler is on by default. Two reference workflows come registered:
