...
The MCP smoke check needs no venv; `./scripts/mcp_check.sh` runs inside the container.
...
Expected. Mint the key at [os.agno.com](https://os.agno.com): **Connect OS** → **Live** → **Settings** → **OS & Security** → **Token-Based Authorization (JWT)**, then paste the full PEM. To do it later, skip the prompt, add `JWT_VERIFICATION_KEY` to `.env.production`, and run `./scripts/railway/env-sync.sh`.
  </Accordion>
