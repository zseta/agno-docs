...
<Warning>`down.sh` deletes the Railway project: the `agent-os` service, the pgvector database, and its volume, including all data. It also comments out the stale `AGENTOS_URL` in your env files so a future `up.sh` starts clean.</Warning>
