...
The reference [Starter](/deploy/introduction). [`agno-agi/agentos-railway`](https://github.com/agno-agi/agentos-railway) runs locally on Docker and deploys AgentOS with PostgreSQL (pgvector) to [Railway](https://railway.com) with one script.
...
| Your product | REST API with 80+ endpoints, explorable at `/docs`. |
...
Token-Based Authorization gives you three things: no public access (requests without a valid token are rejected), per-request identity (`user_id`, `session_id`, and custom claims parsed from the token), and granular permissions (user tokens run agents and read their own sessions; admin tokens read everything). The same middleware protects `/mcp`: clients authenticate with a bearer token, either a JWT minted at os.agno.com or a service-account PAT (`uvx agno connect` mints one automatically). To opt out inside a private VPC behind another auth layer, set `authorization=False` in `app/main.py`.
...
For auto-deploy on merge, connect your repo in the Railway dashboard: your project → `agent-os` service → **Settings** → **Source** → **Connect Repo**, deploy branch `main`. Env changes still sync with `./scripts/railway/env-sync.sh`.
