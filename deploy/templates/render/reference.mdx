...
| `RUNTIME_ENV` | No | `prd` | `dev` disables JWT. Compose sets it for local. Never set it to `dev` in an env file that syncs to Render, or production deploys unauthenticated. |
...
| `MCP_CONNECT_SECRET` | No | - | OAuth consent secret (16+ chars) for connecting claude.ai and ChatGPT to `/mcp`. `up.sh` generates one on deploy and writes it to `.env.production`. |
