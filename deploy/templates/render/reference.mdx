...
| `RENDER_API_KEY` | Deploy scripts | - | Drives the Render API in `scripts/render/`. Read from your environment or env file; `env-sync.sh` never pushes `RENDER_*` keys to the service. |
...
The service is on the `free` plan, which sleeps between requests. The in-process scheduler and MCP streams need an always-on instance. Set `plan: starter` (or higher) in `render.yaml` and push.
  </Accordion>
</AccordionGroup>
