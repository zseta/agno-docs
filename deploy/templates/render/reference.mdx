...
| `RUNTIME_ENV` | No | `prd` | `dev` disables JWT. Compose sets it for local. Never put `dev` in an env file that `env-sync.sh` pushes to Render, or production serves unauthenticated. |
