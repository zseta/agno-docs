...
Waits for the service to be live, pins `AGENTOS_URL` to the real service URL so scheduled jobs reach the platform (`render.yaml` can't reference its own URL), and pauses for a JWT verification key.
  </Step>
...
For claude.ai and ChatGPT on the web, add `https://<onrender-domain>/mcp` as a custom connector in the chat app's connector settings and sign in with OAuth.
  </Step>
