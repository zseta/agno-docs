...
The web service runs on the `starter` plan, the cheapest that never sleeps; the in-process scheduler and MCP streams need an always-on instance. It runs as a single instance by design. Two instances double-fire every cron.
  </Step>
