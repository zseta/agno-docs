...
sidebarTitle: "Deploy"
description: "Set up the Render Starter with a coding-agent prompt, run it on Docker, and deploy to Render with a Blueprint and one wiring script."
---

[`agno-agi/agentos-render`](https://github.com/agno-agi/agentos-render) is the Render [Starter](/deploy/introduction): it runs locally on Docker and deploys AgentOS with managed PostgreSQL (pgvector) to [Render](https://render.com) with a Blueprint and one wiring script.
...
> **Prerequisites:** A [Render](https://render.com) account with access to your copy of the repo. A `RENDER_API_KEY` (dashboard → **Account Settings** → **API Keys**) in your environment or env file for the scripts.
...
The web service runs on the `starter` plan, the cheapest that never sleeps; the in-process scheduler and MCP streams need an always-on instance. It runs as a single instance by design, because two instances would double-fire every cron.
  </Step>
...
Waits for the service to be live, pins `AGENTOS_URL` to the real service URL so scheduled jobs reach the platform, and pauses for a JWT verification key.
  </Step>
...
For claude.ai and ChatGPT on the web, register `https://<your-onrender-domain>/mcp` as a remote MCP server and sign in with OAuth.
  </Step>

  <Step title="Confirm it's live">
The script prints your service URL. Open `https://<your-onrender-domain>/docs` to confirm the API is serving. Logs live in the dashboard: `agent-os` → **Logs**.
  </Step>
</Steps>
...
`autoDeploy: true` is on in `render.yaml`: pushing to your deploy branch redeploys automatically. Render builds the pushed branch; local uncommitted changes never deploy.

To re-run a build without a new commit:

```bash
./scripts/render/redeploy.sh
```

Env changes still sync with `./scripts/render/env-sync.sh`.

## Environment variables

The variables you touch during setup. Full table in the [reference](/deploy/templates/render/reference#environment-variables).

| Variable | Required | Description |
|----------|----------|-------------|
| `OPENAI_API_KEY` | Yes | Models and embeddings. The Blueprint prompts for it at launch. |
| `RENDER_API_KEY` | Deploy scripts | Drives the Render API in `scripts/render/`. Never synced to the service. |
...
| `AGENTOS_URL` | No | Scheduler base URL. `up.sh` pins it to your onrender.com URL. |
