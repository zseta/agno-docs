...
| Task | Command |
| :--- | :--- |
| Deploy updates | `ag infra patch prd:aws` |
| Stop deployment | `ag infra down prd:aws` |
| Build production image | `ag infra up prd:docker` |
| Force rebuild image | `ag infra up prd:docker -f` |
| Update task definition | `ag infra patch -e prd -i aws -n td` |
| Redeploy ECS service | `ag infra patch -e prd -i aws -n service` |
| Restart local containers | `ag infra restart -e dev -c docker -t container` |
| Recreate local resources | `ag infra up dev:docker -f` |

<Warning>
  `ag infra down prd:aws` removes all AWS resources, including the database. Back up your data first.
</Warning>
...
3. Rebuild development image: `ag infra up -e dev -i docker -t image`
4. Rebuild production image: `ag infra up -e prd -i docker -t image`
