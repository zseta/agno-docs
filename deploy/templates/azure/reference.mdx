...
| `MCP_CONNECT_SECRET` | No | - | If set (16+ chars), `/mcp` serves its own OAuth authorization server so claude.ai and ChatGPT can connect; connecting asks for this secret on a consent page. `up.sh` generates it into `.env.production` on deploy. |
