...
Both start empty and grow with use. Mention that Josh from Anthropic shared a new RLM paper, and Scout adds Josh to the CRM, parses the paper into the wiki, and links them. See [Knowledge wiki and CRM](/tutorials/scout/knowledge-and-crm) for how both systems work.
...
Scout is now running at `http://localhost:8000`. The [Setup guide](/tutorials/scout/setup) has the full walkthrough.
...
For production, swap the knowledge wiki to a Git-backed repo so pages survive container restarts. [Deploy to Railway](/tutorials/scout/deploy-to-railway) covers that, plus connecting the repo to Railway for auto-deploys on every push.
...
2. Create the Slack app from the manifest in [Connect to Slack](/tutorials/scout/connect-slack).
