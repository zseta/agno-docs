...
| claude.ai and ChatGPT | Hosted sessions can't reach localhost. Deploy to production first, then add `https://<cloud-run-url>/mcp` as a remote connector; the platform serves its own OAuth and you approve the consent page with your `MCP_CONNECT_SECRET`. |
...
For claude.ai and ChatGPT on the web, the platform serves its own OAuth authorization server on `/mcp`; `up.sh` generated the login secret for it (`MCP_CONNECT_SECRET` in `.env.production`) during deploy. Add `https://<cloud-run-url>/mcp` as a custom connector in the chat app's connector settings and approve the consent page with that secret. Leave the form's optional OAuth fields (client ID / client secret) empty; the platform registers the app automatically.
  </Step>
