...
The Cloud Run service is `agent-os`, the Cloud SQL instance is `agentos-db`, and the Artifact Registry repo is `agentos`. Scripts target the current gcloud project and `us-central1`; override with `GCP_PROJECT_ID` and `GCP_REGION`. For setup and the first deploy, see [Deploy](/deploy/templates/gcp/deploy).
