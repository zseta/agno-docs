...
| `MCP_CONNECT_SECRET` | No | generated by `up.sh` | If set (16+ chars, e.g. `openssl rand -base64 32`), `/mcp` becomes its own OAuth 2.1 authorization server for claude.ai and ChatGPT; connecting asks for this secret on a consent page. Requires `AGENTOS_URL`. `up.sh` generates it into `.env.production` on deploy. PAT and JWT bearers keep working alongside. |
