...
<Note>
In `TeamMode.tasks`, streaming also emits dedicated task events (`TeamTaskCreated`, `TeamTaskUpdated`, `TeamTaskIterationStarted`) when `stream_events=True`. See [Task Mode Streaming Events](/examples/teams/modes/tasks/streaming-events).
</Note>
