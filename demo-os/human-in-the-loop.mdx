...
Three demo agents exercise the HITL surface end-to-end. Different patterns, same primitives.
...
Pre-hooks catch bad input before the model sees it (PII, injection, moderation). Post-hooks catch bad output and write the audit log. The agent never sees raw PII; the audit log captures every run.
...
PII gets masked in place. Injection attempts get blocked outright. Both are common to use together.
...
Next: [Multi-Agent Teams →](/demo-os/multi-agent-teams/text-to-sql)
