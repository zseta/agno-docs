...
The MCP agent uses MCP as a tool. [Scout's `MCPContextProvider`](/deploy/templates/scout/overview) goes one step further: any MCP server becomes a registered context. Scout calls `query_mcp_<slug>` and a sub-agent answers from that server's tools. Same protocol, different ergonomic layer.
