...
The [MCP agent](https://github.com/agno-agi/demo-os/blob/main/agents/mcp/agent.py) is twenty lines of code that does one thing: answer questions about Agno using the official docs MCP server. No local knowledge base, no embeddings to maintain, no ingest pipeline. The docs site exposes an MCP endpoint, the agent connects to it, the answers stay current.
...
For most third-party services, MCP wins. For first-party data (your database, your APIs), a `@tool` function is more direct.
...
The MCP agent uses MCP as a tool. [Scout's `MCPContextProvider`](/tutorials/scout/next-steps#connect-mcp-servers) goes one step further: any MCP server becomes a registered context. Scout calls `query_mcp_<slug>` and a sub-agent answers from that server's tools. Same protocol, different ergonomic layer.

Source: [`agents/mcp/agent.py`](https://github.com/agno-agi/demo-os/blob/main/agents/mcp/agent.py)

Next: [Context Providers →](/demo-os/context-providers)
