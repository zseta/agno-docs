...
The [Contacts agent](https://github.com/agno-agi/demo-os/blob/main/agents/contacts/agent.py) is a relationship intelligence agent. A mini CRM that remembers people, the facts you tell it about them, who knows whom, and what's happening between them.
...
For most agents, `enable_agentic_memory=True` is enough. Single bucket of facts. Agent writes via tools.

`LearningMachine` is right when the shape of memory matters:

- A relationship graph (Contacts) needs entities + edges, not flat strings.
- A long-running personal agent (Pal) needs separate "what I know about you" vs "what I know about your domain".
- A self-improving SQL agent (Dash) needs separate "validated queries" vs "error patterns I've fixed".

When in doubt, start with `enable_agentic_memory=True`. Move to `LearningMachine` once flat memory feels too lossy.
...
Next: [Learning →](/demo-os/learning)
