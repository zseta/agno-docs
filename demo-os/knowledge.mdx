...
```python
# From a directory
dash_knowledge.insert(path="knowledge/tables/")

# From a URL
dash_knowledge.insert(url="https://example.com/article")

# Programmatically
dash_knowledge.insert(
    name="MRR definition",
    text_content="MRR is sum of active subscriptions excluding trials.",
...
By default, `Knowledge` chunks long content and embeds each chunk with `text-embedding-3-small`. The embedder is set on the vector database. The chunking strategy is set on the reader that ingests the content:
