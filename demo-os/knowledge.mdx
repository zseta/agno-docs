...
By default, `Knowledge` chunks long content and embeds each chunk with `text-embedding-3-small`. You set the embedder on the vector database and the chunking strategy on the reader that ingests the content:
