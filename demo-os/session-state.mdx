...
Session state fills the "things the user is working on right now" slot. Memory fills "things about the user". Dependencies fill "config for this exact request".
...
If state should survive the user closing the chat and coming back next week, that's not session state. That's memory or a database table the agent reads from.
...
Next: [Human-in-the-Loop →](/demo-os/human-in-the-loop)
