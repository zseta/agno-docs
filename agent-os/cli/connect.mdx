...
Each client gets its own [service account](/agent-os/security/authorization/service-accounts), so a leaked or retired client is revoked without touching the others:
...
`--json` prints one JSON document with the discovered OS, per-client results, and the exit code. Exit codes: `0` all connected, `1` nothing connected, `2` usage error, `3` partial success. In `--json` mode the CLI never prompts; set `AGNO_ADMIN_TOKEN` before the run.
