...
Re-runs are safe. Entries that still verify are left alone, broken or stale ones are rotated (interactive runs confirm first), and `--skip-existing` never touches anything that exists.
