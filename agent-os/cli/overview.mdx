...
The CLI ships with the SDK. Installing `agno` 2.7 or later puts `agno` on your PATH.
...
The probe covers `AgentOS.serve()`'s default port (7777), common bump-up ports, and a bare uvicorn setup (8000). An AgentOS on any other host or port needs `--url` or `AGENTOS_URL`.

## Developer Resources

- [agnoctl reference](/reference/cli/agnoctl): full flag tables, environment variables, exit codes, and JSON output schemas
- [Starter templates](/deploy/templates)
