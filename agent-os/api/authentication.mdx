...
Machine callers (coding agents, CI, chat apps) authenticate with opaque bearer tokens instead of JWTs:

```bash
curl --location 'http://localhost:7777/agents/my-agent/runs' \
    --header 'Authorization: Bearer agno_pat_...' \
    --header 'Content-Type: application/x-www-form-urlencoded' \
    --data-urlencode 'message=Your query here'
```

Tokens start with `agno_pat_` and work in every authentication mode, including alongside `OS_SECURITY_KEY`. The account's stored scopes are enforced on every request, and runs are attributed to the `sa:<name>` principal. Mint one with `agno tokens create <name>` or `POST /service-accounts`. See [Service Accounts](/agent-os/security/authorization/service-accounts).
