...
[Service account](/agent-os/security/authorization/service-accounts) principals (`sa:<name>`) self-scope regardless of the `user_isolation` flag. The scoping decision runs in this order:
...
A machine token stamps its sessions and memories with its own principal, so an unscoped default would read every user's history. For a cross-user token, mint one with the admin scope.
