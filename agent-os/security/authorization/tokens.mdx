...
JWTs identify human callers. For machine callers, mint a [service account](/agent-os/security/authorization/service-accounts) token instead; `agno tokens create <name>` returns an `agno_pat_...` token sent in the same `Authorization` header.
