...
description: "Machine identities authenticated by opaque agno_pat_ tokens: mint, scope, and revoke."
---
...
The `201` response contains the plaintext `token` once. Store it now; it can never be retrieved again. [`agno tokens`](/agent-os/cli/tokens) wraps this API for the terminal.
...
Service accounts require a database on your AgentOS (`AgentOS(db=...)`). Tokens are stored there, next to sessions and memories. Minting requires a real credential; anonymous requests on an open instance get `401`.
