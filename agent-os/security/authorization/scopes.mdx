...
<Tabs>
  ...
  <Tab title="Interfaces (A2A)">
    A2A routes reuse the `agents`, `teams`, and `workflows` scopes. Per-resource scopes like `agents:my-agent:run` authorize A2A paths the same way they do REST paths. Routes below assume the default `/a2a` prefix; a custom `A2A(prefix=...)` is gated identically under its own prefix.
