...
`forwardedProps.user_id` attributes a run to a user for anonymous callers only. Authenticated callers are pinned to their token's principal, and a client-supplied `user_id` is ignored. See [Scopes](/agent-os/security/authorization/scopes) for the full route-to-scope mapping.
