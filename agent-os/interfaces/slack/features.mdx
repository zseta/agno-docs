...
| [Context provider](/agent-os/context-providers) | Query and update Slack from any agent | `SlackContextProvider()` |
...
Each Slack thread maps to one session. DMs work the same way. Conversations persist across server restarts.
