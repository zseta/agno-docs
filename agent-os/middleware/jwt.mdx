...
<Note>
Construction requires at least one credential source: a JWT key source (`verification_keys`, `jwks_file`, or their environment variables), `validate=False`, a `security_key`, or a `service_account_verifier`. `authorization=True` additionally requires a JWT source (verification keys, a JWKS file, or `validate=False` for unverified dev mode).
</Note>
