...
When a user triggers a tool decorated with `@approval`, the run pauses and a pending record is persisted to the database. An admin resolves the request via the AgentOS Control Panel or the API, and the run can then be continued.
...
| `@approval(type="audit")` | **Non-blocking.** The run pauses only for the tool's HITL step. A resolved audit record is created after the step completes. Requires an HITL flag on `@tool()`, such as `requires_confirmation`. | Compliance logging, activity auditing |
