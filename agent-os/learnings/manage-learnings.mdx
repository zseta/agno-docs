...
Scoping follows the framework's opt-in [user isolation](/agent-os/security/authorization/user-isolation) contract (`AuthorizationConfig(user_isolation=True)`). Admins, requests with isolation disabled, and requests without a JWT are unscoped and have full access. For a scoped (non-admin) caller:
