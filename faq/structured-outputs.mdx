...
If the model class supports it, Agno agents use Structured Outputs by default. The schema (Pydantic or JSON Schema) is passed to the model provider's API, and the response is guaranteed to strictly follow it. No missing fields, invalid enum values, or inconsistent formatting.
...
The agent returns a `User` object instead of free-form text. Structured Outputs suit high-confidence extraction tasks: entity extraction, content generation for UI rendering, and similar.
