...
# Extras combine
pip install -U 'agno[os,postgres,sql,pgvector]'
```
...
| `fastmcp` not installed | `AgentOS(enable_mcp_server=True)` | `pip install 'agno[mcp]'` |
| `sqlalchemy` not installed | `SqliteDb` | `pip install 'agno[sqlite]'` |
| `sqlalchemy` not installed | `PostgresDb` | `pip install 'agno[sql,postgres]'` |
