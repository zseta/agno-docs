...
description: "Switch models mid-session safely within one provider; cross-provider switches can break on message history format."
---

Agno supports 45+ model providers. Switching models within the same provider is safe, but switching between providers on the same session can break because stored message history formats differ between providers.
