...
Connects coding agents to a running AgentOS over MCP: discover, mint one service account per client, write each client's MCP config, read the entry back, and verify it with a real MCP `initialize` + `tools/list` handshake. Requires the AgentOS to have `enable_mcp_server=True`.
