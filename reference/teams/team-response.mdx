...
| `followups` | `List[str]` | `None` | Suggested followup prompts generated after the run, when followups are enabled |
| `model_provider_data` | `Dict[str, Any]` | `None` | Model provider specific metadata|
...
| `last_checkpoint_at_message_index` | `int` | `None` | Index into `messages` at the most recent checkpoint write. Set when `checkpoint="tool-batch"` persists mid-run state |
| `forked_from_run_id` | `str` | `None` | Run ID this run was forked from |
| `forked_from_message_index` | `int` | `None` | Message index in the source run where the fork was created |
| `forked_from_session_id` | `str` | `None` | Session ID this run was originally created in. Set when a session is forked and preserved across nested forks |
| `regenerated_from` | `str` | `None` | Run ID of the immediate predecessor this run was regenerated from |
