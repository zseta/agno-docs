...
The AgentOS authentication middleware. Handles JWTs, service account tokens (`agno_pat_...`), the internal service token, and the OS security key, with optional RBAC (Role-Based Access Control) for JWTs. `JWTMiddleware` is a kept alias for the manual `app.add_middleware(JWTMiddleware, ...)` setup path.
