...
You can use multiple reasoning toolkits together for multi-domain reasoning. Tool names must stay unique, so disable overlapping `think`/`analyze` entries (or rename the later ones) to prevent silent overrides:
