...
To use `MoviePyVideoTools`, you need to install `moviepy` and its dependency `ffmpeg`. The example below also uses `OpenAITools`, which needs `openai`:

```shell
uv pip install moviepy ffmpeg openai
```
