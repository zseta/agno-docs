...
`SchedulerTools` wraps `ScheduleManager` so an agent can create, list, enable, disable, and delete cron-based schedules in response to prompts like _"Run a daily health check at 9am"_.
...
Requires a running AgentOS server with `scheduler=True` for scheduled tasks to execute.
